...
export default function EditLibraryModal({ open, library, onClose, onUpdate }: EditLibraryModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.error || '更新媒体库失败'
      setError(msg)
    } finally {
      setSubmitting(false)
    }
            ...
            <div>
              ...
              {showAdvanced && (
                <div className="mt-4 space-y-6 animate-slide-up">
                  ...
                  {/* ———— 2. 文件过滤 ———— */}
                  <div>
                    <h4 className="text-sm font-semibold mb-2.5" style={{ color: 'var(--text-primary)' }}>
                      文件过滤
                    </h4>
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        style={{
                          background: advanced.enable_file_filter
                            ? 'linear-gradient(135deg, var(--neon-blue), var(--neon-purple))'
                            : 'transparent',
                          border: advanced.enable_file_filter ? 'none' : '2px solid var(--border-hover)',
                          boxShadow: advanced.enable_file_filter ? '0 0 8px var(--neon-blue-25)' : 'none',
                        }}
                      >
                      ...
                      <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>排除小于</span>
                      <input
                        type="number"
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="mt-1 text-xs leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>
                        实时监控媒体文件夹的变化，自动检测并同步新增、修改或删除的媒体文件。
                      </p>
                    </div>
                    <ToggleSwitch
